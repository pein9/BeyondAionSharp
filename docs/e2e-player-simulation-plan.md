...
| B5 | **Partly resolved in P2-02.** `Aion.Bots` now owns the real client crypt, framing and opcode transforms; CM writers and SM decoders remain P2-04/P2-05. `AionXorCipher` is explicitly marked as an unrelated legacy helper. | Bots have the wire envelope but cannot act or perceive packet bodies yet. | `tests/Aion.Bots/Protocol/GamePacketCodec.cs`; `BotGamePacketCodecTests.cs` |
