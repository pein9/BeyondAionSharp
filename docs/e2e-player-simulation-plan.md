...
| B5 | **Partly resolved through P2-04.** `Aion.Bots` owns the real client crypt, framing, opcode transforms and all Appendix B CM writers; SM decoders remain P2-05. `AionXorCipher` is explicitly marked as an unrelated legacy helper. | Bots can form actions but cannot perceive packet bodies yet. | `tests/Aion.Bots/Protocol/`; `BotGameClientPacketWriterTests.cs` |
