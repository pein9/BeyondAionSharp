...
| 83 | Soak quest combat waits forever after a normal interrupted cast (harness protocol omission) | Java `PlayerController.cancelCurrentSkill` cancels and clears casting, broadcasts `SM_SKILL_CANCEL` and sends `STR_SKILL_CANCELED`; `Skill.endCast` returns after cancellation without `SM_CASTSPELL_RESULT` (`ce54b7931`). LIVE mixed50-2h-c b01 receives exactly this sequence for Flame Bolt 1282 at 05:02:54.582 UTC | Open: `LiveSoakQuests.KillAsync` waits only for the result until its 1,800-second activity timeout. Nineteen other starter journeys persist; the stalled bot's connection/pings are not progress. Handle result or cancellation with a bounded wait and retry normal interruptions, preserving timing and real combat. No production interruption change or allowance |
