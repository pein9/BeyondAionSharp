...
| 109 | Assertion-HTTP request is reset during a Docker-hosted 500-subject persistence check | mixed500-docker-c b110 fails `VerifyOfflineAsync` with `HttpRequestException` / socket 104 while writing the request; paired b109 propagates it. LS/CS logs are empty, GS only its existing startup allowance. No Java analogue for the C# admin HTTP infrastructure | **OPEN.** Keep raw socket/runtime samples and failed evidence. No sampled listen overflow/drop, SYN retransmission, TCP memory abort or backlog drop; counters are aggregate and cannot attribute this connection. Connection reuse/idle close remains a hypothesis, not a proven cause. No suppression, retry, allowance or success conversion |
