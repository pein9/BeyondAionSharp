...
| 81 | Solo PvP soak oracle excludes ordinary randomly triggered AP buffs (harness reward-contract omission) | Java `PvpService.doReward` calls `EventService.onPvpKill` before reward distribution; `EventBuffHandler` applies shipped skill 10549 with 2% kill-trigger probability, and `Rates.AP_PVP` applies `AP_BOOST` before long truncation (`ce54b7931`) | Open: mixed50-2h-a stops after decoded SM_ABNORMAL_STATE confirms skill 10549 and 330 AP instead of the oracle's unboosted 300. Victim loss and kill counts match. Retain the production event and add observed-effect-aware assertions; do not disable events, accept an arbitrary reward range, or allowlist this fingerprint. This failed run is not two-hour capacity evidence |
