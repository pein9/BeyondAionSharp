...
| B9 | **Partly resolved through P4-05.** `Rnd` now accepts execution-context and process-wide deterministic seeds while its production default remains thread-local; the known gameplay `Random` bypasses use it. .NET still randomizes string hashing per process, and string-keyed and concurrent collections can enumerate in a different order every run. | The random stream is replayable, but P4-06 must stabilize order before whole-world SIM runs are replayable. | `Commons/Utils/Rnd.cs`; `SpawnGroup.cs`; `ItemRandomBonusTable.cs`; `FastMath.cs` |
