...
| B4 | **Partly resolved through P4-06.** The virtual scheduler now surfaces faults, cannot move time backwards, reports virtual delay correctly and has strict disposal. Deterministic mode routes movement sequentially, periodic-manager rearming, `NetFlusher` and the shutdown countdown through it; Quartz (P4-07/P4-08) and `PacketProcessor` still bypass it. | Whole-server SIM still needs cron routing and the remaining deterministic-thread routing in Phase 5. | `VirtualThreadPool.cs`; `ThreadPoolManager.cs`; `MoveTaskManager.cs`; `NetFlusher.cs`; `ShutdownHook.cs` |
