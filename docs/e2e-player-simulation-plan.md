...
| 79 | Concurrent watchers can replace later ledger edits/counts with a stale startup snapshot (harness evidence defect) | No Java analogue; `KnownProblemLedger.Load/RecordRun/Save` atomically replaces its in-memory dictionary but does not merge concurrent writers | Observed while PvP and alternate-port telemetry diagnostics overlapped. Per-run evidence remains intact. Shared-ledger concurrent merge is still open; reconcile after active writers stop and fix before concurrent-run acceptance |
