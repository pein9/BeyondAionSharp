...
- [ ] **P10-09** [LIVE] L — Turn the open journeys in `docs/Deep-Port-Audit-Remediation-Tracker.md` into LIVE
  ...
  **In progress:** the watcher has an explicit second-GS topology option with independent log/heartbeat
  ...
  starter inventory (§7/135). These are bugs, not allowances; correction and full target-login/data/control
  acceptance remain pending. Only two setup bots were used, with normal scheduler timing and owned cleanup.
...
| 134 | Transfer clone does not load target account passports before saving (shared upstream defect) | After the D19 relay/wire corrections, `p10-09-transfer-relay-a` reaches target `CMT_CHARACTER_INFORMATION.ReadInfo` and persists a partial clone, then `PlayerService.StorePlayer` calls `AccountPassportsDAO.StorePassport` with a null passport list (fingerprint `92a0c151`). Java/C# `AccountService.loadAccount/LoadAccount` do not load passports; normal `PlayerService.getPlayer/GetPlayer` does, but the clone uses `newPlayer/NewPlayer` instead | Open scoped transfer follow-up under D19. Load the target account's own persisted passport/stamp state before final save; do not invent empty state, import source-account rewards, or allowlist the failure. Task remains ACTIVE, subject account is disabled, source/control rows stay unchanged. Java reference `ce54b7931`: AccountService, PlayerService, CMT_CHARACTER_INFORMATION and AccountPassportsDAO; runtime receipt in validation document |
| 135 | Starter-class transfer duplicates character-creation inventory (shared upstream defect) | The same diagnostic has 13 source inventory rows and 26 target rows: every starter item is duplicated. Both Java/C# CMT call `PlayerService.newPlayer/NewPlayer`, which supplies starter items for classes with creation data, then append the serialized source inventory. This is a data assertion failure even without a dedicated error log | Open scoped transfer follow-up under D19. Separate cloning initialization from new-character starter grants, preserve ordinary character creation, and compare normalized full inventory in the accepted journey. Do not delete duplicate rows as test cleanup/repair or accept task DONE alone. Java reference `ce54b7931`: services/player/PlayerService.newPlayer and services/transfers/CMT_CHARACTER_INFORMATION.readInfo |
