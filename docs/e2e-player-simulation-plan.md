...
| B4 | **Partly resolved through P4-07.** The virtual scheduler now surfaces faults, cannot move time backwards, reports virtual delay correctly and has strict disposal. Deterministic mode routes movement sequentially, periodic-manager rearming, `NetFlusher`, shutdown countdowns and cron jobs through it; `AbstractCronTask` construction still needs the P4-08 drain protocol and `PacketProcessor` still bypasses it. | Whole-server SIM still needs safe cron-singleton construction and the remaining deterministic-thread routing in Phase 5. | `VirtualThreadPool.cs`; `ThreadPoolManager.cs`; `MoveTaskManager.cs`; `NetFlusher.cs`; `ShutdownHook.cs`; `CronService.cs` |
