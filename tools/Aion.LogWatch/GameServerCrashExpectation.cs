...
internal sealed class GameServerCrashExpectation
{
	...
	private GameServerCrashExpectation(Plan plan) => this.plan = plan;

	internal static GameServerCrashExpectation Load(string json, string run, string project, DateTimeOffset now)
	{
	...
	{
		if (project != plan.Project || container != plan.ContainerId || service != "gameserver") return false;
	...
	internal bool ExpectsHeartbeatGap(string server, DateTimeOffset now) =>
		server == "gs" && !Complete && DiedUtc is { } died && now >= died && now <= plan.RecoveryDeadlineUtc;

	internal string? Failure(DateTimeOffset now, bool final) =>
		DiedUtc == null && (final || now > plan.KillDeadlineUtc) ? "The planned game-server SIGKILL was not observed before its deadline." :
		!Complete && (final || now > plan.RecoveryDeadlineUtc) ? "The killed game server did not restart with a fresh heartbeat before its deadline." : null;
}
