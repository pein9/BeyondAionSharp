...
/// <summary>
/// Java parity: dao/BonusPackDAO (@author Estrayl, Neon). JDBC DAO over bonus_packs (per-account bonus-pack receiving player).
...
{
    private static readonly ILogger log = AionLog.For(nameof(BonusPackDAO));

    private const string UPDATE_QUERY = "REPLACE INTO `bonus_packs` (`account_id`, `receiving_player`) VALUES (?,?)";
    private const string SELECT_QUERY = "SELECT `receiving_player` FROM `bonus_packs` WHERE `account_id`=?";
    ...
    public static int LoadReceivingPlayer(int accountId)
    {
        ...
        {
            log.LogError(e, "[BONUS_PACK] Error loading received player id on account id " + accountId);
            return int.MaxValue;
        }
    }
    ...
    public static bool StoreReceivingPlayer(int accountId, int playerId)
    {
        ...
        {
            log.LogError(e, "[BONUS_PACK] Error saving received player id " + playerId + " on account id " + accountId);
            return false;
        }
    }
}
