    ...
    public class RespawnTask : Aion.Commons.Lang.Runnable
    {
        ...
        private bool TryRegisterOnEventEndTask()
        {
            ...
            foreach (Aion.GameServer.Services.Event.Event activeEvent in Aion.GameServer.Services.Event.EventService.GetInstance().GetActiveEvents())
            {
                ...
                if (activeEvent.GetEventTemplate().GetSpawns().GetTemplates().Any(m => m.GetMapId() == spawnTemplate.GetWorldId() && m.GetSpawns().Any(spawn => spawn.GetNpcId() == spawnTemplate.GetNpcId() && spawn.IsCustom == true)))
