...
public sealed class PlayerProtectionActiveTaskStopTriggerJavaTraceArtifactReaderTests
{
	...
	[Fact]
	public void CmLevelReadyCallerOriginArtifacts_RecordStartProtectionBeforeWorldSpawn()
	{
		var artifact = ParseCmLevelReadyCallerOriginArtifact();

		Assert.Equal(1, artifact.SchemaVersion);
		Assert.Equal("cm-level-ready-start-protection-before-world-spawn", artifact.Scenario);
		Assert.Equal("cm_level_ready_before_world_spawn", artifact.RuntimeFacts.ExpectedReturnReason);
		Assert.Contains("CM_LEVEL_READY.runImpl", artifact.JavaSources);
		...
		var cleanup = artifact.Traces.Single(trace => trace.Phase == "start_cast_target_cleanup");
		...
		var worldSpawnPending = artifact.Traces.Single(trace => trace.Phase == "world_spawn_pending");

		Assert.NotNull(caller.CallerOrigin);
		Assert.Equal("cm_level_ready_before_world_spawn", caller.CallerOrigin!.CallerName);
		Assert.Equal("CM_LEVEL_READY", caller.CallerOrigin.CallerClass);
		Assert.Equal("runImpl", caller.CallerOrigin.CallerMethod);
		Assert.Equal(53, caller.CallerOrigin.StartProtectionLine);
		Assert.True(caller.CallerOrigin.StartsProtectionBeforeWorldSpawn);
		Assert.Equal(64, caller.CallerOrigin.WorldSpawnLine);
		Assert.Equal("start_protection_before_world_spawn", caller.CallerOrigin.Ordering);
		Assert.False(startGuard.Player.ProtectionActiveBefore);
		Assert.True(visualSet.Player.ProtectionActiveAfter);
		Assert.Contains("BLINKING", visualSet.Player.VisualStateAfter);
		Assert.Equal("cancel_cast_and_remove_target", cleanup.ActionBranchName);
		Assert.NotNull(fanout.Fanout);
		Assert.Equal("SM_PLAYER_STATE", fanout.Fanout!.PacketName);
		Assert.NotNull(schedule.Scheduler);
		Assert.Equal(60000, schedule.Scheduler!.DelayMillis);
		Assert.True(schedule.Scheduler.NewFutureStored);
		Assert.True(caller.EventSeq < startGuard.EventSeq);
		Assert.True(startGuard.EventSeq < visualSet.EventSeq);
		Assert.True(visualSet.EventSeq < cleanup.EventSeq);
		Assert.True(cleanup.EventSeq < fanout.EventSeq);
		Assert.True(fanout.EventSeq < schedule.EventSeq);
		Assert.True(schedule.EventSeq < worldSpawnPending.EventSeq);
	...
	[Fact]
	public void DelayedTeleportFallbackArtifacts_RecordMissingInstanceSpawnWithoutPositionSet()
	{
		var artifact = ParseDelayedTeleportFallbackArtifact();

		Assert.Equal(1, artifact.SchemaVersion);
		Assert.Equal("delayed-teleport-missing-instance-fallback-spawns-without-position-set", artifact.Scenario);
		Assert.Equal("delayed_teleport_missing_instance_fallback_spawn", artifact.RuntimeFacts.ExpectedReturnReason);
		Assert.Contains("TeleportService.SpawnTask.run", artifact.JavaSources);
		...
		var teleportTaskRemove = artifact.Traces.Single(trace => trace.Phase == "teleport_task_remove");
		var spawnTaskRun = artifact.Traces.Single(trace => trace.Phase == "spawn_task_run");
		var fallbackGuard = artifact.Traces.Single(trace => trace.Phase == "fallback_guard");
		var fallbackPacket = artifact.Traces.Single(trace => trace.Phase == "fallback_player_info_packet");
		var fallbackSpawn = artifact.Traces.Single(trace => trace.Phase == "fallback_world_spawn");

		Assert.NotNull(animationDone.CallerOrigin);
		Assert.Equal("delayed_teleport_missing_instance_fallback_spawn", animationDone.CallerOrigin!.CallerName);
		Assert.Equal("CM_TELEPORT_ANIMATION_DONE", animationDone.CallerOrigin.CallerClass);
		Assert.Equal("runImpl", animationDone.CallerOrigin.CallerMethod);
		Assert.Null(animationDone.CallerOrigin.StartProtectionLine);
		Assert.False(animationDone.CallerOrigin.StartsProtectionBeforeWorldSpawn);
		Assert.Equal(505, animationDone.CallerOrigin.WorldSpawnLine);
		Assert.False(animationDone.Player.Spawned);
		Assert.NotNull(teleportTaskRemove.Scheduler);
		Assert.True(teleportTaskRemove.Scheduler!.OldFuturePresent);
		Assert.Equal("TeleportService.SpawnTask.run", teleportTaskRemove.Scheduler.CallbackMethod);
		Assert.False(spawnTaskRun.Player.Spawned);
		Assert.False(fallbackGuard.Player.Spawned);
		...
		Assert.True(animationDone.EventSeq < teleportTaskRemove.EventSeq);
		Assert.True(teleportTaskRemove.EventSeq < spawnTaskRun.EventSeq);
		Assert.True(spawnTaskRun.EventSeq < fallbackGuard.EventSeq);
		Assert.True(fallbackGuard.EventSeq < fallbackPacket.EventSeq);
	...
	private static ProtectionStopTriggerJavaTraceArtifact ParseCmLevelReadyCallerOriginArtifact()
	{
		const string json = """
			{
			  "schemaVersion": 1,
			  "javaCommit": "abcdef1",
			  "scenario": "cm-level-ready-start-protection-before-world-spawn",
			  "runtimeFacts": {
			    "serverFlavor": "java",
			    "packetName": "CM_LEVEL_READY",
			    "playerObjectId": 1001,
			    "worldId": 210010000,
			    "expectedReturnReason": "cm_level_ready_before_world_spawn"
			  },
			  "javaSources": [
			    "CM_LEVEL_READY.runImpl",
			    "PlayerController.startProtectionActiveTask",
			    "ThreadPoolManager.schedule",
			    "CreatureController.addTask"
			  ],
			  "traces": [
			    {
			      "schemaVersion": 1,
			      "traceId": "cm-level-ready-start-protection-before-world-spawn-001",
			      "eventSeq": 0,
			      "phase": "caller_enter",
			      "packetName": "CM_LEVEL_READY",
			      "returnReason": "cm_level_ready_before_world_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000000000,
			      "monotonicNanos": 6000000,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "CM_LEVEL_READY.java",
			      "javaLine": 53,
			      "player": {
			        "objectId": 1001,
			        "spawned": false,
			      ...
			      "callerOrigin": {
			        ...
			        "ordering": "start_protection_before_world_spawn"
			      },
			      "actionBranchName": "call_start_protection_active_task"
			    },
			    {
			      "schemaVersion": 1,
			      "traceId": "cm-level-ready-start-protection-before-world-spawn-001",
			    ...
			    },
			    {
			      ...
			      "traceId": "cm-level-ready-start-protection-before-world-spawn-001",
			      "eventSeq": 6,
			      "phase": "world_spawn_pending",
			      "packetName": "CM_LEVEL_READY",
			      "returnReason": "cm_level_ready_before_world_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000000006,
			      "monotonicNanos": 6000600,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "CM_LEVEL_READY.java",
			      "javaLine": 64,
			      "player": {
			      ...
			      "actionBranchName": "world_spawn_after_start_protection"
			    }
			  ],
			  "notes": [
	...
	private static ProtectionStopTriggerJavaTraceArtifact ParseDelayedTeleportFallbackArtifact()
	{
		const string json = """
			{
			  "schemaVersion": 1,
			  "javaCommit": "abcdef1",
			  "scenario": "delayed-teleport-missing-instance-fallback-spawns-without-position-set",
			  "runtimeFacts": {
			    "serverFlavor": "java",
			    "packetName": "CM_TELEPORT_ANIMATION_DONE",
			    "playerObjectId": 1001,
			    "worldId": 301390000,
			    "expectedReturnReason": "delayed_teleport_missing_instance_fallback_spawn"
			  },
			  "javaSources": [
			    "CM_TELEPORT_ANIMATION_DONE.runImpl",
			    "TeleportService.SpawnTask.run",
			    "InstanceService.instanceExists",
			  ...
			  "traces": [
			    {
			      "schemaVersion": 1,
			      "traceId": "delayed-teleport-missing-instance-fallback-spawns-without-position-set-001",
			      "eventSeq": 0,
			      "phase": "animation_done_enter",
			      "packetName": "CM_TELEPORT_ANIMATION_DONE",
			      "returnReason": "delayed_teleport_missing_instance_fallback_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002000,
			      "monotonicNanos": 12000000,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "CM_TELEPORT_ANIMATION_DONE.java",
			      "javaLine": 34,
			      "player": {
			      ...
			      "scheduler": null,
			      "callerOrigin": {
			        "callerName": "delayed_teleport_missing_instance_fallback_spawn",
			        "callerClass": "CM_TELEPORT_ANIMATION_DONE",
			        "callerMethod": "runImpl",
			        "callerSourceFile": "CM_TELEPORT_ANIMATION_DONE.java",
			        "callerLine": 40,
			        ...
			        "worldSpawnLine": 505,
			        "spawnedBeforeStart": false,
			        "ordering": "animation_done_runs_spawn_task_fallback_spawn_without_position_set"
			      },
			      "actionBranchName": "animation_done_packet"
			    },
			    {
			      ...
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002001,
			      "monotonicNanos": 12000100,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "CM_TELEPORT_ANIMATION_DONE.java",
			      "javaLine": 36,
			      "player": {
			    ...
			    },
			    {
			      ...
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002002,
			      "monotonicNanos": 12000200,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "TeleportService.java",
			      "javaLine": 498,
			      "player": {
			    ...
			    },
			    {
			      ...
			      "traceId": "delayed-teleport-missing-instance-fallback-spawns-without-position-set-001",
			      "eventSeq": 3,
			      "phase": "fallback_guard",
			      "packetName": "CM_TELEPORT_ANIMATION_DONE",
			      "returnReason": "delayed_teleport_missing_instance_fallback_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002003,
			      "monotonicNanos": 12000300,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "TeleportService.java",
			      "javaLine": 503,
			      "player": {
			      ...
			      "scheduler": null,
			      "callerOrigin": null,
			      "actionBranchName": "instance_missing_fallback"
			    },
			    {
			      "schemaVersion": 1,
			      "traceId": "delayed-teleport-missing-instance-fallback-spawns-without-position-set-001",
			      "eventSeq": 4,
			      "phase": "fallback_player_info_packet",
			      "packetName": "CM_TELEPORT_ANIMATION_DONE",
			      "returnReason": "delayed_teleport_missing_instance_fallback_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002004,
			      "monotonicNanos": 12000400,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "TeleportService.java",
			      "javaLine": 504,
			      "player": {
			      ...
			      "scheduler": null,
			      "callerOrigin": null,
			      "actionBranchName": "send_player_info_before_fallback_spawn"
			    },
			    {
			      "schemaVersion": 1,
			      "traceId": "delayed-teleport-missing-instance-fallback-spawns-without-position-set-001",
			      "eventSeq": 5,
			      "phase": "fallback_world_spawn",
			      "packetName": "CM_TELEPORT_ANIMATION_DONE",
			      "returnReason": "delayed_teleport_missing_instance_fallback_spawn",
			      "stopCalled": false,
			      "expectsStopProtectionCall": false,
			      "wallTimeEpochMillis": 1760000002005,
			      "monotonicNanos": 12000500,
			      "timestampIsParityKey": false,
			      "javaSourceFile": "TeleportService.java",
			      "javaLine": 505,
			      "player": {
			      ...
			      "callerOrigin": {
			        ...
			        "worldSpawnLine": 505,
			        "spawnedBeforeStart": false,
			        "ordering": "fallback_player_info_before_world_spawn_then_return"
			      },
			      "actionBranchName": "world_spawn_fallback_return"
			    }
			  ],
			  "notes": [
	...
	private sealed record ProtectionStopTriggerTraceRow(
		...
		ProtectionStopTriggerCallerOriginSnapshot? CallerOrigin,
		string ActionBranchName);
	...
	private sealed record ProtectionStopTriggerCallerOriginSnapshot(
		string CallerName,
		string? CallerClass,
		string? CallerMethod,
		string CallerSourceFile,
		int CallerLine,
		int? StartProtectionLine,
		bool StartsProtectionBeforeWorldSpawn,
		int? WorldSpawnLine,
		bool SpawnedBeforeStart,
		string? Ordering);
}
