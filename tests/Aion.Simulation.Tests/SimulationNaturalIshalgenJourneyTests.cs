...
public sealed partial class SimulationFastScenarioTests
{
	...
	public async Task NaturalIshalgenPriestCompletesFrozenJourneyWithoutSetup()
	{
		...
		// Monsters whose aggro circle the Priest stood in without their attacking: not waited for again.
		...
		NaturalJourneyRunContext RunContext() => new(combatTrace.Run, "SIM-natural-ishalgen", fixture.Seed,
			typeof(SimulationFastScenarioTests).Assembly.GetCustomAttributes(false)
				.OfType<System.Reflection.AssemblyInformationalVersionAttribute>().Single().InformationalVersion,
			typeof(SimulationFastScenarioTests).Module.ModuleVersionId.ToString(), fixture.Clock.NowMillis);
		...
		void WriteFailure(string kind, Exception failure)
		{
			...
			try
			{
				...
				string path = new NaturalJourneyFailure(kind, session.CurrentStep, session.CurrentAction,
					...
					{ ServerProblems = System.Text.Json.JsonSerializer.SerializeToElement(policy.SnapshotProblems()) }
		...
		try
		{
			...
			Assert.Contains(2000, session.Api.World.CompletedQuestIds);
			var player = fixture.World.GetPlayer(session.CharacterId);
			Assert.Equal(PlayerClass.PRIEST, player.GetPlayerClass()); // Read-only identity oracle.
			...
			long journeyStart = fixture.Clock.NowMillis;
			long lastProgressObservation = -1000;
			session.AfterSynchronize = () =>
			{
				if (!session.Api.World.LoginStateObserved || fixture.Clock.NowMillis - lastProgressObservation < 1000) return;
				lastProgressObservation = fixture.Clock.NowMillis;
				checkpoint = NaturalJourneyCheckpoint.Capture(session.Api.World, session.CharacterId,
					session.ConnectionGeneration, contract, session.CurrentPosition);
				progress.Observe(checkpoint, TimeSpan.FromMilliseconds(fixture.Clock.NowMillis - journeyStart));
			};
			const float DeathSpotAvoidance = 12f;
			int reconnects = 0;
			string? relogAt = Environment.GetEnvironmentVariable("NI08_RELOG_AT");
			string? stopAt = Environment.GetEnvironmentVariable("NI08_STOP_AT");
			...
			for (int sequence = 1; sequence <= 100; sequence++)
			{
				try
				{
					...
					{
						Assert.Contains(questId, session.Api.World.CompletedQuestIds);
				...
				{
					if (relogAt != null) Assert.True(relogInjected, "Requested NI-08 interruption was never exercised before checkpoint.");
				...
				catch (Exception failure) when (NaturalJourneyReconnect.IsConnectionLoss(failure) && reconnects < 3 && !token.IsCancellationRequested)
				{
					...
					player = fixture.World.GetPlayer(session.CharacterId);
			...
			async Task CompleteQ2104Async()
			{
				...
				if (QuestStatus(2104) == 3) Assert.Equal(3, ItemCount(session.Api.World, 182203104));
			...
			{
				if (stopAt != null) Assert.True(stopInjected, "Requested NI-08 checkpoint was never exercised.");
				if (relogAt != null) Assert.True(relogInjected, "Requested NI-08 interruption was never exercised.");
					...
					for (int guardClears = 0; guardClears <= NaturalApproachProgress.MaximumAttempts; guardClears++)
					{
								...
								"New client-observed hazards exceeded the bounded replan budget.")
						{
							...
							if (await TryClearObservedBlockerAsync(seen?.Position ?? anchor.Position, seen?.ObjectId))
							{
								...
								progress.Observe(Distance(beforeApproach, session.CurrentPosition),
		...
		catch (Exception failure)
		{
			WriteFailure(failure is TimeoutException ? "stall" : failure is OperationCanceledException ? "cancelled" :
				failure is SimulationLogPolicyException ? "server-policy" : "execution", failure);
