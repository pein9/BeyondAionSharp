...
{
    public static void Verify(int characterId, IReadOnlyCollection<BotInventoryItem> clientItems, long clientKinah, JsonElement response)
    {
        ...
        var serverItems = inventory.GetProperty("items").EnumerateArray().Select(row => new BotInventoryItem(
            ...
            row.GetProperty("equipmentSlot").GetUInt16(), row.GetProperty("cloth").GetBoolean())).ToArray();
